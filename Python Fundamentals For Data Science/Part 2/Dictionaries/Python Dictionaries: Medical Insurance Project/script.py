...
for name, record in medical_records.items():
    print(name + " is a " + str(record["Age"]) + " year old " + record["Sex"] + " " + record[
        "Smoker"] + " with a BMI of " + str(record["BMI"]) + " and insurance cost of " + str(record["Insurance_cost"]))


def update_medical_records(name, age, sex, BMI, Children, Smoker, Insurance_cost):
    medical_record_dict[name] = {"Age": age, "Sex": sex, "BMI": BMI, "Children": Children, "Smoker": Smoker,
                                 "Insurance_cost": Insurance_cost}
    medical_records.update(mdeical_records_dict)
